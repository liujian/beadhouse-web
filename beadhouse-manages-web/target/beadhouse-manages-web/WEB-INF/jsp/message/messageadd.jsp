	...
	function dataverify(){
	 ...
	 function savemess(){
     	     ...
     	     if("${obj.id}"==null||"${obj.id}"==""){
     		    $("#itemAddForm").attr("action","<%=basePath%>/MessageCenter/messageaddSD");
     		     loading('保存中，请稍等...');
     		    itemAddEditor.sync();
	     	...
	     	$("#itemAddForm").attr("action","<%=basePath%>/MessageCenter/updatemessageSD?keywords="+keywords+"");
	     		 loading('保存中，请稍等...');
	     		   itemAddEditor.sync();
		        $("#itemAddForm").submit();
	     	} 
	 ...
	 function goback(){ 
		 var keywords= encodeURI(encodeURI("${keywords}"));
		 window.location.href="<%=basePath%>/MessageCenter/messageSDList?keywords="+keywords+"";
	 }
...
<body>
   ...
   <c:if test="${obj.id==null}"> 
    ...
    <div class="formtitle"> 
         <c:if test="${obj.id==null}"> 
          <span>时贷活动内容推送新增</span>
         </c:if>
        <c:if test="${obj.id!=null}"> 
          <span>时贷活动内容推送编辑</span>
         </c:if>
    </div>
    <form action="/HelpCenter/addAnswerandContent" method="post" id="itemAddForm">
	    <ul class="forminfo">
	    	...
	    	<li><label>&nbsp;</label><input name="id" id="id" type="hidden"  value="${obj.id}" /><input name="createuser" id="createuser" type="hidden"  value="${obj.createuser}" /> <input  type="button" onclick="dataverify()" class="btn" value="保存"/>
