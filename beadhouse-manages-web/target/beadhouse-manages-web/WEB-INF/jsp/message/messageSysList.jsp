...
$(document).ready(function(){
	...
	function query(){
		   ...
		   $("#searchForm").attr(
					"action",
					"<%=basePath%>/MessageCenter/messageSysList");
			loading('正在查询，请稍等...');
           $("#searchForm").submit();
	}
	function addsave(){
...
<body>
    ...
    <div class="rightinfo">
           <form id="searchForm" action="/MessageCenter/messageSysList" method="post">
	        <ul class="seachform">
		     ...
		     <li><label>&nbsp;</label>
		     <z:permission value="pms:messagesyswh:view">
	         ...
	         <li><label>&nbsp;</label>
	          <z:permission value="pms:messagesyswh:add">
    ...
    <c:forEach items="${HelpCenterlist}" var="obj" varStatus="sta">
	    <tr>
	    	...
	    	<td>
	    	<%--  <z:permission value="pms:messagesyswh:edit">
			   	<a href="javascript:void(0)" onclick="contentEditer(${obj.id});" class="tablelink">编辑</a>
			 </z:permission>   --%> 	
			  <z:permission value="pms:messagesyswh:delete">
<%-- 			    <a href="<%=basePath%>/MessageCenter/deleteMessageSys?id=${obj.id}&currentpage=${pagenation.currentpage}&rowCount=${pagenation.rowCount}" class="tablelink"> 删除</a>--%>		     
	...
	function deletemessage(id,currentpage,rowCount){
		 $.jBox.confirm("确定删除该吗？", "删除提示", function (v, h, f) {
	         if (v == 'ok'){
	        	   $.ajax({
	   		            type:"post",
	   		            url:"<%=basePath%>/MessageCenter/deleteMessageSys?id="+id+"&currentpage="+currentpage+"&rowCount="+rowCount,
	...
	function contentEditer(id){
		var keywords= encodeURI(encodeURI($("#keywords").val()));
 		 window.location.href="<%=basePath%>/MessageCenter/messageSysupdatepage?id="+id+"&keywords="+keywords+"";
	}
	function goPage(page){  
	    document.getElementById("currentpage").value=page;   
	    var keywords= encodeURI(encodeURI($("#keywords").val()));
		$("#pagenation").attr("action","<%=basePath%>/MessageCenter/messageSysList?keywords="+keywords+"");
    	$("#pagenation").submit();
 	} 
