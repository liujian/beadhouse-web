...
function onCheck(event, treeId, treeNode){
  ...
  function onBodyDown(event) {
	...
	function dataverify(){
	 ...
	 function goback(){ 
		 var keywords= encodeURI(encodeURI("${keywords}"));
		 window.location.href="<%=basePath%>/Author/authorityList?keywords="+keywords+"";
	 }
...
<body>
   ...
   <c:if test="${obj.id==null}"> 
    ...
    <form action="<%=basePath%>/Author/authorityAdd" method="post" id="searchForm">
	   <ul class="forminfo">
	    	...
	    	<li>
	    		<label><font color="#f00">*</font>权限标识</label>
	    		 <div class="span6">
	    		...
	    		<label>菜单ID</label>
                    ...
                    <div class="combo_tree" style="width:60%">
                        <div class="input-append combo_input_box">
                            <input  type="text" id="menuname" name="menuname" style="height: 30px;" value="${obj.menuname}"  class="add-on-after-input combo_input" autocomplete="off">
                            <span class="add-on add_for_tree" id="menuBtn" onclick="showMenu(); return false;">∨</span>
                            <input  type="hidden" id="menuid" name="menuid" style="height: 30px;" value="${obj.menuid}"  class="add-on-after-input combo_input" autocomplete="off">
                        </div>
	    	...
	    	<li><label>&nbsp;</label>
	    	<div class="span6">
	    	<input name="id" id="id" type="hidden"  value="${obj.id}" /><input  type="button" onclick="dataverify()"  class="btn btn-primary" value="保存"/>
	    &nbsp;&nbsp; <input type="button" onclick="goback()" id="add"   class="btn btn-primary" value="返回"/> 
	        </div>
	     </li>
	    </ul>
    </form>
